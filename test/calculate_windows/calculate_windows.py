...
def main():
    ...
    ap.add_argument(
        "-o", "--binned_freq", help="", default=sys.stdout, type=argparse.FileType("wt")
    )
    ...
    for index in bed_df.index:
        ...
        seq = np.arange(int(start), int(stop) + int(args.slide), int(args.slide))
        ...
        for i in range(len(seq) - window_size + 1):
            ...
            print(RM_start, RM_stop, file=sys.stderr)
    ...
    for idx_2 in keep_window.index:
        ...
        args.binned_freq.write("\t".join([chrom, pos1, pos2, freq, f"{idx_2}\n"]))
